...
const props = defineProps(['items', 'page', 'icon'])
    ...
    <template v-for="(childItem, index) in items" :key="index">
      <li class="flex">
        ...
        <router-link
          ...
          class="group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-blue-200 duration-300 ease-in-out hover:text-white"
          :class="{
